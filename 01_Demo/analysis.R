...
# Subset Data in R -- Filter a 2-dimentinal R data structure
?subset()
filter_table2 <- subset(demo_table2, price > 10000 & drive == "4wd" & "clean" %in% title_status) #filter by price and drivetrain
View(filter_table2)

# Sample Data in R -- Random picking
?sample()
...
# Transform using mutate() function
library(tidyverse)
?mutate()
demo_table <- demo_table %>% mutate(Mileage_per_Year=Total_Miles/(2020-Year),IsActive=TRUE) #add columns to original data frame
View(demo_table)

# Group Data --group_by() function
...
# Reshape Data -- gather() and spread()
?gather()
...
cor(mtcars$hp,mtcars$qsec) # cor() function use to calculate Pearson correlation coefficient

used_cars <- read.csv('used_car_data.csv',stringsAsFactors = F) #read in datase
head(used_cars)
